...
#%% Command Line Inputs
parser = argparse.ArgumentParser(description="DA-LSTM model training for \"Spatial-Temporal Analysis of Groundwater Well Features from Neural Network Prediction of Hexavalent Chromium Concentration\".")
...
f_final_train_attns, f_final_eval_train_attns, f_final_eval_test_attns, t_final_train_attns, t_final_eval_train_attns, t_final_eval_test_attns = model.do_train(label_scaler)
model.eval()
...
plt.savefig(directory + 'model_iter_losses', bbox_inches='tight')
plt.close()

fig2 = plt.figure()
plt.semilogy(range(len(model.epoch_losses)), model.epoch_losses)
plt.savefig(directory + 'model_epoch_losses', bbox_inches='tight')
plt.close()
...
fig3 = plt.figure()
plt.figure(figsize=(10, 6))
line = plt.plot(range(len(real[train_split:])), real[train_split:], alpha=0.5, label='True Features', color='green')
title = 'Predicted - Test | Sequence'
line2 = plt.plot(range(len(y_pred)), y_pred, alpha=0.5, label='Predicted Features', color='red')
plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
plt.title(title)
plt.savefig(directory + 'seq_pred_test' , bbox_inches='tight')
plt.close()
...
with open('scalers.pkl', 'wb') as f:
    pickle.dump({"scaler": scaler, "label_scaler": label_scaler}, f)

if f_final_train_attns[-1].shape[0] != f_final_train_attns[0].shape[0]:
    nan_arr = np.full((batchsize - f_final_train_attns[-1].shape[0], f_final_train_attns[-1].shape[1], f_final_train_attns[-1].shape[2]), np.nan)
    f_final_train_attns[-1] = np.concatenate((f_final_train_attns[-1], nan_arr), axis=0)
np.save(directory + 'f_final_att_trains.npy', np.array(f_final_train_attns))

np.save(directory + 'f_final_eval_train_attns.npy', np.array(f_final_eval_train_attns))
np.save(directory + 'f_final_eval_test_attns.npy', np.array(f_final_eval_test_attns))
np.save(directory + 'f_final_att_tests.npy', np.array(f_final_test_attns))

if t_final_train_attns[-1].shape[0] != t_final_train_attns[0].shape[0]:
    nan_arr = np.full((batchsize - t_final_train_attns[-1].shape[0], t_final_train_attns[-1].shape[1], t_final_train_attns[-1].shape[2]), np.nan)
    t_final_train_attns[-1] = np.concatenate((t_final_train_attns[-1], nan_arr), axis=0)
np.save(directory + 't_final_att_trains.npy', np.array(t_final_train_attns))

np.save(directory + 't_final_eval_train_attns.npy', np.array(t_final_eval_train_attns))
np.save(directory + 't_final_eval_test_attns.npy', np.array(t_final_eval_test_attns))
